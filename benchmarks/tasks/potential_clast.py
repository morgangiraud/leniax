...
def make_run_fn(rng_key, config, multiplier):
    ...
    state = jax.random.uniform(subkeys[0], [N_init] + world_size + [C])

    max_k_per_channel = 1
    K = jax.random.uniform(subkeys[1], [2 * R, 2 * R, 1, C * max_k_per_channel])
    K /= K.sum()
    ...
    for dim in K.shape[:2]:
        if dim % 2 == 0:
            ...
            pad_l += [(dim // 2, dim // 2)]
    padding = tuple(pad_l + [(0, 0)])

    true_channels = tuple([i for i in range(C)])

    K.block_until_ready()

    def bench_fn():
        potential = leniax_core.get_potential(state, K, padding, true_channels, ('NHWC', 'HWIO', 'NHWC'))
