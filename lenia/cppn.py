...
def create_mlp(input_dim, num_channels, output_dim, omega=30, rng_seed=None):
    modules = []
    modules.append(Dense(num_channels[0], W_init=siren_init_first(), b_init=bias_uniform()))
    modules.append(Sine(omega))

    for nc in num_channels:
        modules.append(Dense(nc, W_init=siren_init(omega=omega), b_init=bias_uniform()))
        modules.append(Sine(omega))

    modules.append(Dense(output_dim, W_init=siren_init(omega=omega), b_init=bias_uniform()))
...
if __name__ == '__main__':
    ...
    # Tooling
    model = ImageCPPN(layer_outputs_sizes, n_channel, omega, rng_seed)
    ...
    data = {"input": xy, "output": target_cell}

    for itr in range(2**13):
        optimizer.step(data)
        if itr % 64 == 0:
